...
static void _panel_props_object(struct nk_context *ctx)
{
	if (!object_selected)
		return;
	...
	nk_property_float(ctx, "X", -INFINITY, &object_selected->trans[3][0],
		   INFINITY, 0.001f, 0.001f);
	nk_property_float(ctx, "Y", -INFINITY, &object_selected->trans[3][1],
		   INFINITY, 0.001f, 0.001f);
	nk_property_float(ctx, "Z", -INFINITY, &object_selected->trans[3][2],
		   INFINITY, 0.001f, 0.001f);
	...
	const char *obj_name = object_selected->name;
	const uint32_t obj_flags = object_selected->flags;
	uint32_t *obj_flagsp = &object_selected->flags;
...
void panel_props(void *ctxin)
{
	...
	nk_layout_row_dynamic(ctx, 30, 1);
	_panel_props_load_and_export(ctx);
	_panel_props_object(ctx);
